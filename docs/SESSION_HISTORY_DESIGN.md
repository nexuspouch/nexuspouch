...
- P2：语义召回（分块 embedding，接向量搜索）→ `session_recall`；✅ 已落地；
- P3：agent-bridge 网关实时旁路捕获 → 覆盖非文件型 agent；✅ 已落地；
- 隐私红线：默认 private + 索引前敏感信息清洗，清洗级别用户可选。

## 0.5 产品定位与 UI（主打卖点）

**一句话卖点**：Nexuspouch —— 你所有 agent 会话的统一管理中心：
本机采集、统一搜索、永不丢失。

- **楔子定位**：会话管理是进入平台的窗口，不是产品终点——UI 必须始终展示
  "这只是第一扇窗"（其他设备同步、语义回忆、产物/记忆空间入口）；
- **单机即有值**：不需要用户先理解多设备/协作愿景，装好就能看到并搜索自己
  所有历史会话（Claude Code / Codex 等）；
- **零配置采集**：适配器自动扫描各 agent 会话目录，首次启动即回填全部历史；
- **隐私加分**："会话留在你自己的机器上管理"是天然卖点（会话含密钥/秘密，
  用户本就不想上云）；
- **数据引力**：会话是向量检索（H）与记忆空间（I）的语料，楔子直接喂大平台。

### MVP 三屏

1. **总览屏**：按 agent / 项目 / 日期列出全部会话，显示摘要、时长、消息数；
2. **详情屏**：干净的对话流（消息 / 工具调用折叠），可版本回看
   （`versions.list` 即会话时间线）；
3. **搜索屏**：跨所有 agent 全文搜索（先 FTS5，后接语义 `session_recall`），
   结果直达来源。

- 形态：节点自带的 Web 管理界面（admin UI v2 或独立 Web 应用，只读起步）；
- 每会话 manifest.summary（"这个会话解决了什么"）驱动总览与搜索标题。
